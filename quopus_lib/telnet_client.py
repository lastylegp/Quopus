...
class _TerminalWidget(QWidget):
    ...
    def keyPressEvent(self, ev: QKeyEvent):
        ...
        if key == Qt.Key.Key_Backspace:
            # PETSCII terminals (real C64 BBSes) expect 0x14
            # (INST/DEL) for both backspace and delete - the C64
            # has only that one editing key. Send it regardless
            # of what backspace_byte was configured for ASCII
            # terminals, so a user who never touches the
            # 'Backspace sends' setting still gets a working DEL
            # key when they switch the terminal type to PETSCII.
            if getattr(self.screen, "encoding", "") == "petscii":
                self._send(b"\x14")
            else:
                self._send(bytes([self.backspace_byte]))
            return
        if key == Qt.Key.Key_Delete:
            # Same story for Delete: on a PETSCII terminal it
            # collapses into the single 0x14 INST/DEL too.
            # Modern terminals get the ANSI CSI 3~ sequence.
            if getattr(self.screen, "encoding", "") == "petscii":
                self._send(b"\x14")
            else:
                self._send(b"\x1b[3~")
    ...
    def _build_ui(self, s: TelnetSession):
        ...
        self._preset_specs = [
            ...
            ("PETSCII BBS", (
                ...
                "  Backspace = PETSCII INST/DEL (0x14, what every\n"
                "  C64 BBS expects), no local echo."),
                {
                    "cmb_proto": ("data", "telnet"),
        ...
        # 0x14 is PETSCII INST/DEL - the C64's only editing key.
        # PETSCII BBSes universally expect this byte.
        self.cmb_bs.addItem("PETSCII INST/DEL (0x14)", "20")
        for i in range(self.cmb_bs.count()):
            if self.cmb_bs.itemData(i) == s.backspace_sends:
                self.cmb_bs.setCurrentIndex(i)
                break
        f_beh.addRow("Backspace sends:", self.cmb_bs)
        # When the user flips the terminal type to PETSCII, default
        # the Backspace-sends combo to 'PETSCII INST/DEL (0x14)'
        # automatically. The user can still pick something else
        # afterwards if they really want to - we only nudge them
        # to the sensible default, no hard lock-in.
        def _on_term_type_changed(text: str) -> None:
            if text == "petscii":
                # Only flip if the user hadn't already set a
                # PETSCII-aware byte, so we don't override a
                # deliberate manual choice.
                cur = self.cmb_bs.currentData()
                if cur in ("127", "8"):
                    for i in range(self.cmb_bs.count()):
                        if self.cmb_bs.itemData(i) == "20":
                            self.cmb_bs.setCurrentIndex(i)
                            break
        self.cmb_term.currentTextChanged.connect(
            _on_term_type_changed)
